...
class Myalgen extends CI_Controller {
	...
	var $ruang, $sesi, $hari;
	var $dosen, $ajar, $kesediaan, $kesediaanAjar;
	...
	{
		set_time_limit(0);		
		$ta 	= $this->input->post('id_tahun_ajaran');	
		$this->cek_jadwal($ta);
		$this->proses_core_generate($ta);
	}
	...
	function proses_core_generate($ta = null){
		...
		$this->kesediaanAjar 		= $this->get_data_kesediaan_ajar($data['id_tahun_ajaran']);
		$this->kesediaan			= $this->get_data_kesediaan($data['id_tahun_ajaran']);
		$arr_kesediaan 				= $this->kesediaan;
		...
		$proses 		= $this->proses_testing($ajar, 1500, 20, $arr_kesediaan, $ajar_kosong,$this->look_ajar_dosen);
	...
	#------------------------------------PROSES--------------------------------------------------------#
		...
		function proses_testing($data, $iterasi, $individu, $arr_kesediaan, $arr_kesediaan_kosong, $look_dosen){
			...
			$populasi_awal = $this->pembangkitanPopulasiAwal($data, $individu);
			...
			for ($i=0; $i < $iterasi; $i++) { 
				...
				$mutasi 		= $this->mutasi_7_byFitness($offspring, $individu, $fitness);
		...
		function cek_jadwal($ta){
			# Cek Panjang Gen
			$SQL 		= "SELECT ((select count(id_hari) from hari) * (select count(id_sesi) from sesi) * (select count(id_ruang) from ruang)) AS panjangGen";
			$panjangGen = $this->mymodel->query($SQL)->row_array()['panjangGen'];

			$jumlahAjar = $this->mymodel->get_where('ajar', array('id_tahun_ajaran'=>$ta))->num_rows(); 

			# 1 MataKuliah = 3 sks
			if (($jumlahAjar*3) < $panjangGen) {
				# Jumlah Data Ajar terlalu banyak sehingga tidak dapat di proses karena kekurangan ruang
				$selisih = intval($panjangGen) - intval($jumlahAjar);
				$this->add_ajar_kosong($selisih, $ta);
			}
		}
		...
		function view($solusi_fitness, $parameter){
			...
			if ($this->mymodel->cek_data("jadwal_kuliah",array('id_tahun_ajaran'=>$data['id_tahun_ajaran']))){							
				$data['jadwalTersedia']= true;
			} else {
				$data['jadwalTersedia']= false;
			}	
		...
		function add_ajar_kosong($loop, $ta){
			for ($i=0; $i < $loop; $i++) {
				$data = array(
						'id_dosen' 		 => '1',
						'id_mata_kuliah' => '1',
		...
		# data kode ajar yang dengan kesediaan dosen pengajar berdasarkan hari & sesi
		function get_data_kesediaan_ajar($ta){
			...
			foreach ($hari as $k_hari => $v_hari) {
				...
				foreach ($sesi as $k_sesi => $v_sesi) {
					...
					$result_dosen = $this->mymodel->getKesediaanAjar($v_hari, $v_sesi)->result_array();
					$arr_kesediaan_ajar[$v_hari][$v_sesi] = array_column($result_dosen, "id_ajar");
				}
			
			}

			return $arr_kesediaan_ajar;
		}
	...
	#------------------------------------POPULASI------------------------------------------------------#
		...
		# terakhir digunakan : 13/12/2015 21:40
		...
		function pembangkitanPopulasiAwal($array, $loop){
			# untuk melihat jadwal - dosen pengajar			
			$look 		= $this->look_ajar_dosen;
			$all_ajar 	= $array;
			...
			for ($i=0; $i < $loop; $i++) { 
				...
				foreach ($array as $k_gen => $value) {
					...
					# Sesi prioritas
					# ambil data ajar yang bersedia berdasarkan hari & sesi
					$dosen_bersedia = $this->kesediaanAjar[$hari][$sesi];							
					# hilangkan data ajar yang sudah ada di dalam populasi awal agar tidak terjadi duplikat data ajar
					$dosen_bersedia = array_diff($dosen_bersedia, $pop[$i]);
					...
					if (in_array($sesi, $daftar_sesi_prioritas)){													
						if (count($ajar) == 0){
							...
							# cek apakah ada dosen yang bersedia
							if (count($dosen_bersedia) > 0){
								# random data dari data ajar bersedia
								$dosen_terpilih = $dosen_bersedia[array_rand($dosen_bersedia)];	
								
								// if (in_array($look[$dosen_terpilih], $ar_cek_bentrok)){
									
								// 	if (count($ar_cek_bentrok_ajar) > 0){
								// 		$dosen_bersedia2 = array_diff($dosen_bersedia, $ar_cek_bentrok_ajar);				
								// 	}
									
									# cek apakah masih ada dosen bersedia yang tersisa
									if (count($dosen_bersedia) > 0){
										$dosen_terpilih = $dosen_bersedia[array_rand($dosen_bersedia)];																				
									} else {
										$dosen_terpilih = $kosong[array_rand($kosong)];									
									}

								// } else {
								// 	$ar_cek_bentrok[] = $look[$dosen_terpilih];
								// 	$ar_cek_bentrok_ajar[] = $dosen_terpilih;
								// }

							} else {
								# masukkan jadwal kosong
								$dosen_terpilih = $kosong[array_rand($kosong)];
							}

							$pop[$i][] = $dosen_terpilih;
						}
					} else {
						if (in_array($sesi, $daftar_sesi_wajib_kosong)){
							$pop[$i][] = $kosong[array_rand($kosong)];
	...
	#------------------------------------MUTASI--------------------------------------------------------#
		...
		function mutasi($array, $jumlah_populasi){
			...
			foreach ($titik_mutasi as $key => $value) {
				...
				if ($gen_terpilih!=$panjang_gen_individu-7) {
		...
		# Swap Mutation
		function mutasi_7_byFitness($array, $jumlah_populasi, $fitness){
			// echo "<h2>Mutasi</h2>";
			#$panjang_gen    = count($array)*count($array[0]);
			// $panjang_gen    = ($this->hari->num_rows() * $this->sesi->num_rows() *$this->ruang->num_rows())*$jumlah_populasi;
			$panjang_gen    = ($this->hari->num_rows() * $this->sesi->num_rows() *$this->ruang->num_rows())*count($array);
			...
			$gen_tidak_sesuai = array();
			foreach ($fitness as $key_individu => $individu) {
				foreach ($individu as $key_gen => $gen) {
					if ($gen > 0) {
						// $titik_mutasi[$key_individu][] = $key_gen;
						$gen_tidak_sesuai[] = (count($array[0])*($key_individu)) + $key_gen;
					}
				}
			}

			// echo "Mutation Rate ".$mutation_rate . "=====" . "Gen Tidak Sesuai ".count($gen_tidak_sesuai) . "<br>";
			// print_r($gen_tidak_sesuai);echo "<br>";
			
			$titik_mutasi	= array();
			for ($i=0; $i < $mutation_rate; $i++) { 
				if (count($gen_tidak_sesuai) > 0){
					$titik_mutasi[] = $gen_tidak_sesuai[array_rand($gen_tidak_sesuai)];	
					$gen_tidak_sesuai = array_diff($gen_tidak_sesuai, $titik_mutasi);					
				} else {
					$titik_mutasi[] = rand(0, ($panjang_gen-1));						
				}
			}
...
/* End of file myalgen.php */
/* Location: ./application/controllers/myalgen.php */